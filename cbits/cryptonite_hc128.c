...
{
  printf("cryptonite_hc128_init %d %p\n", sizeof(hc128_ctx), ctx); 
...
void generate_keystream(hc128_ctx* ctx, uint32_t* keystream)  
{
   uint32_t cc,dd;
...
void ECRYPT_keysetup(
  hc128_ctx* ctx, 
  const uint8_t* key, 
  uint32_t keysize,                /* Key size in bits (128+128*i) */ 
  uint32_t ivsize)                 /* IV size in bits  (128+128*i)*/
{ 
  uint32_t i;  

  printf("ctx: %p, key: %p, keysize: %p, ivsize: %p\n", ctx, key, keysize, ivsize);
  
  ctx->keysize = keysize;  
  ctx->ivsize = ivsize;

  /* Key size in bits 128 */ 
  for (i = 0; i < (keysize >> 5); i++) ctx->key[i] = U32TO32_LITTLE (((uint32_t*)key)[i]);
...
void ECRYPT_ivsetup(hc128_ctx* ctx, const uint8_t* iv, uint32_t size)
{ 
    uint32_t i;
	...
	for (i = 0; i < (ctx->ivsize >> 5); i++)  ctx->iv[i] = U32TO32_LITTLE(((uint32_t*)iv)[i]);
  ...
  uint32_t msglen)                /* Message length in bytes. */ 
{
  ...
  for ( ; msglen >= 64; msglen -= 64, input += 64, output += 64)
  {
      ...
      /*for (i = 0; i < 16; ++i)
	      ((uint32_t*)output)[i] = ((uint32_t*)input)[i] ^ U32TO32_LITTLE(keystream[i]); */
